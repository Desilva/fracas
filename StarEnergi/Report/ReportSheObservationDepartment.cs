namespace StarEnergi.Report
{
    ...
    /// </summary>
    public partial class ReportSheObservationDepartment : Telerik.Reporting.Report
    {
        public ReportSheObservationDepartment()
        {
            //
        ...
        private void chart1_NeedDataSource(object sender, EventArgs e)
        {
            ...
            string sql =
                @"SELECT * from observationChart12Department('" + this.ReportParameters[0].Value + "')";
        ...
        private void chart2_NeedDataSource(object sender, EventArgs e)
        {
            ...
            string sql =
                @"SELECT sum(ua) as ua, sum(uc) as uc, sum(sa) as sa, sum(sc) as sc from observationChart12Department('" + this.ReportParameters[0].Value + "')";
            string connectionString = ConfigurationManager.ConnectionStrings["starenergygeo"].ToString();
            SqlDataAdapter adapter = new SqlDataAdapter(sql, connectionString);
            DataSet dataSet = new DataSet();
            adapter.Fill(dataSet);
