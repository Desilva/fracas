namespace StarEnergi.Reporting
{
    ...
    /// </summary>
    public partial class PIR_Report_Summary : Telerik.Reporting.Report
    {
        public PIR_Report_Summary()
        {
            //
        ...
        private void ImprovementIndicatorChart_NeedDataSource(object sender, EventArgs e)
        {
            ...
            left join (select * from pir where YEAR(date_rise) = '" + reportParamaters + "' ) as pir on procedure_reference_list.proc_ref_list = pir.procedure_reference group by proc_ref_list"
        ...
        private void PIRYearToDateChart_NeedDataSource(object sender, EventArgs e)
        {
            ...
            DataTable table = new DataTable();
            SqlDataAdapter adapter = new SqlDataAdapter(@"
            SELECT 'Closed' AS [Status], COUNT(CASE WHEN pir.status = 'VERIFIED' THEN 1 END) as yeartodate FROM pir where YEAR(date_rise) = '" + reportParamaters + "' UNION ALL SELECT 'Raised' AS [Status],COUNT(CASE WHEN pir.status != 'VERIFIED' THEN 1 END) as yeartodate FROM pir where YEAR(date_rise) = '" + reportParamaters + "' UNION ALL SELECT 'Open' AS [Status], COUNT(CASE WHEN DateDiff(day,date_rise,pir.initiator_verified_date) <=0 OR (isnull(pir.initiator_verified_date,'') = '' AND isnull(pir.date_rise,'') <> '') THEN 1 END) as yeartodate FROM pir where month(date_rise) <= month(GETDATE()) UNION ALL SELECT 'Overdue' AS [Status], COUNT(CASE WHEN isnull(pir.initiator_verified_date,'') <> '' AND DateDiff(day,date_rise,pir.initiator_verified_date) > 0 THEN 1 END) as yeartodate FROM pir where month(date_rise) <= month(GETDATE())"
                 , ConfigurationManager.ConnectionStrings["starenergygeo"].ConnectionString);
            adapter.Fill(table);
            Telerik.Reporting.Processing.Chart procChart = (Telerik.Reporting.Processing.Chart)sender;
            Telerik.Reporting.Chart defChart = (Telerik.Reporting.Chart)procChart.ItemDefinition;
        ...
        private void OverdueChart_NeedDataSource(object sender, EventArgs e)
        {
            ...
            left join (Select * from pir where YEAR(date_rise) = '" + reportParamaters + "') as p on p.process_owner = proc_owner_list.ownlist group by ownlist"
                 , ConfigurationManager.ConnectionStrings["starenergygeo"].ConnectionString);
            adapter.Fill(table);
            Telerik.Reporting.Processing.Chart procChart = (Telerik.Reporting.Processing.Chart)sender;
            Telerik.Reporting.Chart defChart = (Telerik.Reporting.Chart)procChart.ItemDefinition;
