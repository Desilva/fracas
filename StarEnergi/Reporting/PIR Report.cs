    ...
    public partial class PIR_Report : Telerik.Reporting.Report
    {
        ...
        private void ImprovementIndicatorChart_NeedDataSource(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter adapter = new SqlDataAdapter(
                   @"SELECT procedure_reference, COUNT(date_rise) as yeartodate FROM pir where YEAR(date_rise) = '"+ reportParamaters +"' group by procedure_reference"
...
//            COUNT(CASE WHEN pir.status = 'VERIFIED' THEN 1 END) as Closed,
//            COUNT(CASE WHEN pir.status != 'VERIFIED' THEN 1 END) as Raised,
//            COUNT(CASE WHEN date_rise > GETDATE() THEN 1 END) as [Open], 
//            COUNT(CASE WHEN date_rise < GETDATE() THEN 1 END) as Overdue 
//            from pir
//            where YEAR(date_rise) = '" + reportParamaters + "'"
//                 , ConfigurationManager.ConnectionStrings["starenergygeo"].ConnectionString);
//            adapter.Fill(table);
//            Telerik.Reporting.Processing.Chart procChart = (Telerik.Reporting.Processing.Chart)sender;
//            Telerik.Reporting.Chart defChart = (Telerik.Reporting.Chart)procChart.ItemDefinition;
...
//            } 
        ...
        private void OverdueChart_NeedDataSource(object sender, EventArgs e)
        {
            ...
            select proc_owner_list.ownlist as process_owner,
	            COUNT(CASE WHEN p.date_rise < GETDATE() THEN 1 END) as Overdue
        ...
        private void OpenChart_NeedDataSource(object sender, EventArgs e)
        {
            ...
            select proc_owner_list.ownlist as process_owner,
	            COUNT(CASE WHEN p.date_rise > GETDATE() THEN 1 END) as [Open]
            from proc_owner_list
            left join (Select * from pir where YEAR(date_rise) = '" + reportParamaters + "') as p on p.process_owner = proc_owner_list.ownlist group by ownlist"
                 , ConfigurationManager.ConnectionStrings["starenergygeo"].ConnectionString);
            adapter.Fill(table);
